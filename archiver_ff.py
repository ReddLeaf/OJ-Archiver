...
HOW IT WORKS
1. Run this script.
2. A Firefox window opens (controlled by the script).
...
Then run:
    python oj_archiver_firefox.py

NOTE ON THE CHROME -> FIREFOX PORT
Chrome's version of this script used the Chrome DevTools Protocol command
"Page.printToPDF" (driver.execute_cdp_cmd(...)), which is Chrome-only.
Firefox's geckodriver doesn't implement CDP. Instead, this version uses
Selenium's standard, cross-browser "print_page()" WebDriver command, which
Firefox supports natively (Selenium 4+). One important gotcha: the W3C
print spec defines page size and margins in *centimeters*, not inches, so
those values are converted below.
"""
...
# Persistent Firefox profile so your login can survive between runs.
# Unlike Chrome's --user-data-dir, Firefox is pointed at a profile
# directory with -profile. If the directory doesn't exist yet, Firefox
# will initialize a fresh profile there the first time it runs.
FIREFOX_PROFILE_DIR = os.path.join(os.getcwd(), ".oj_firefox_profile")
...
# Selenium's print_page accepts 0.1-2.0 for this, same as before.
PDF_SCALE = 0.9
...
def setup_driver():
    ...
    # Firefox has no --start-maximized flag; set an initial window size
    # instead (and/or call driver.maximize_window() after launch).
...
def get_pdf_bytes_for_current_page(driver) -> bytes:
    """Use Selenium's standard print_page() WebDriver command (which
    Firefox implements natively) to print the current page to PDF, and
    return the raw PDF bytes (without writing anything to disk).

    Chrome's CDP printToPDF took paper size/margins in inches; the W3C
    print spec that print_page() talks to uses centimeters instead, so
    the values below are converted from the original inch-based config.
    """
    ...
    print_options.page_width = 8.5 * INCH_TO_CM
    print_options.page_height = 11 * INCH_TO_CM
    print_options.margin_top = 0.4 * INCH_TO_CM
    print_options.margin_bottom = 0.4 * INCH_TO_CM
    print_options.margin_left = 0.4 * INCH_TO_CM
    print_options.margin_right = 0.4 * INCH_TO_CM
    print_options.scale = PDF_SCALE
